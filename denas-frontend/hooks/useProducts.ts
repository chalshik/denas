...
export function useProducts() {
    ...
    try {
      ...
      if (filters.search && filters.search.trim()) {
        ...
        const productsWithDetails: ProductWithDetails[] =
          catalogResponse.items.map((product: ProductCatalog) => ({
            id: product.id,
            name: product.name,
  ...
  const fetchFeaturedProducts = async (
    ...
    apiHook.setLoading(true);
    try {
      const products = await api.get<ProductCatalog[]>("/products/featured", {
        limit,
      });
      ...
      if (productData.images && productData.images.length > 0) {
        try {
          ...
          if (
            ...
            const regularUploadResponse = await api.uploadMultipleFiles(
              productData.images,
              "product-images",
            );
  ...
  const updateProduct = async (
    ...
    try {
      ...
      if (productData.name !== undefined)
        productPayload.name = productData.name;
      if (productData.description !== undefined)
        productPayload.description = productData.description;
      if (productData.price !== undefined)
        productPayload.price = productData.price;
      if (productData.category_id !== undefined)
        productPayload.category_id = productData.category_id;
      if (productData.stock_quantity !== undefined)
        productPayload.stock_quantity = productData.stock_quantity;
      if (productData.availability_type !== undefined)
        productPayload.availability_type = productData.availability_type;
      if (productData.preorder_available_date !== undefined)
        productPayload.preorder_available_date =
          productData.preorder_available_date;
      if (productData.is_active !== undefined)
        productPayload.is_active = productData.is_active;
